...
def generate_station_list_csvs():
    for stem in ("ghcnh-station-list", "lcdv2-station-list"):
        input_path = data_dir / f'noaa_lut/{stem}.txt'
        output_path = data_dir / f'noaa_lut/{stem}.csv'
...
# print("Generating station list CSVs...")
# generate_station_list_csvs()
# print("Done generating station list CSVs.")

# %%
ghcnh_stations = pd.read_csv(data_dir / 'noaa_lut/ghcnh-station-list.csv')
lcdv2_stations = pd.read_csv(data_dir / 'noaa_lut/lcdv2-station-list.csv')

def iata_to_icao(code):
    if code in apd_iata:
        return apd_iata[code]['icao']
    raise ValueError("invalid iata code")


def icao_to_iata(code):
    if code in apd_icao:
        return apd_icao[code]['iata']
...
def iata_to_station_id(dataset, code):
    if code == 'DCA':
        ...
        print("issue with identifying IAD")
        return 'USW00093738'
    elif code == 'MSP':
        print("issue with identifying MSP")
        return 'USW00014922'
    elif code == 'ORD':
        print("issue with identifying ORD")
        return 'USW00094846'
    
    if code in apd_iata:
        lat = apd_iata[code]['lat']
        lon = apd_iata[code]['lon']
        return loc_to_station_id(dataset, lat, lon)
    raise ValueError("invalid iata code")

def icao_to_station_id(dataset, code):
    if code in apd_icao:
        lat = apd_icao[code]['lat']
        lon = apd_icao[code]['lon']
        return loc_to_station_id(dataset, lat, lon)
    raise ValueError("invalid icao code")
...
def dl_noaa(dataset, out_base_dir, station_id, station_name=None, 
            start_year=start_year, end_year=end_year, ext="parquet",
...
def clean_noaa_lcdv2_file(data_path, verbose=False, out_time_zone=None):
    ...
    if out_time_zone is not None:
        ...
        #     df['longitude'].iloc[0]
        # )
        # LCD_LGA_2018-2019.csv
        time_zone = apd_iata[(data_path.stem.split('_')[1])]['tz']
    ...
    data_dir = Path(data_dir, 'cleaned')
...
if __name__ == "__main__":
    if not (data_dir / 'noaa_lut' / "ghcnh-station-list.csv").exists() or not (data_dir / 'noaa_lut' / "lcdv2-station-list.csv").exists():
        print("Generating station list CSVs...")
        generate_station_list_csvs()
        print("Done generating station list CSVs.")

    out_dir = data_dir / 'noaa_lcdv2'
    if not out_dir.exists():
        out_dir.mkdir(parents=True, exist_ok=True)

    run_download = True
    print(f"Downloading LCDv2 data for {len(CoreAirports)} airports from {start_year} to {end_year}...")
    if run_download:
        for apt in CoreAirports.keys():
            if (out_dir / f'LCD_{apt}_{start_year}-{end_year}.csv').exists():
                print(f"Data for {apt} already downloaded, skipping...")
                continue
            else:
                dl_noaa_lcdv2_airport(data_dir=out_dir, code=apt)
    # %%
    # clean the data
    # TODO rerun switching everything into UTC
    for apt in CoreAirports.keys():
        data_path = out_dir / f'LCD_{apt}_{start_year}-{end_year}.csv'
        check_already_cleaned  = out_dir / 'cleaned' / f'lcd_{apt.lower()}_{start_year}-{end_year}_cleaned.csv'
        if check_already_cleaned.exists():
            print(f"Data for {apt} already cleaned, skipping...")
            continue
        elif data_path.exists():
            print(f"Cleaning {data_path}...")
            # try:
            print('Resaving to UTC...')
            clean_noaa_lcdv2_file(data_path, verbose=True, out_time_zone='UTC')
            # except Exception as e:
            #     print(f"Error cleaning {data_path}: {e}")
            #     # TODO: handle error, arrises from things not being in EST (i.e. anything not on the east coast)
            #     continue
        else:
            # print(f"Data file {data_path} does not exist, skipping...")
            raise FileNotFoundError(f"Data file {data_path} does not exist, skipping...")
    # %%